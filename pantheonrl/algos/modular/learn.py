        ...
        for partner_idx in range(self.policy.num_partners):
            ...
            for epoch in range(self.n_epochs):
                ...
                for rollout_data in self.rollout_buffer[partner_idx].get(
                    ...
                    main_probs = th.mean(
                        th.exp(
                            main_logits - main_logits.logsumexp(dim=-1, keepdim=True)
                        ),
                        dim=0,
                    )
                    ...
                    marginal_regularization_loss = th.mean(
                        th.sum(th.abs(main_probs - composed_probs), dim=1)
                    )
                    ###########
