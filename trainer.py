...
def input_check(args):
    ...
    if args.alt_config is None:
        ...
        raise EnvException("Number of partners is different from number of configs")
...
def latent_check(args):
    ...
    for conf in args.alt_config:
        if "context_size" not in conf:
            ...
            raise EnvException(
                "both agents must have similar configs \
...
def generate_ego(env, args):
    ...
    if args.ego == "LOAD":
        ...
        return ModularAlgorithm(
            policy=ModularPolicy, policy_kwargs=policy_kwargs, **kwargs
        )
    else:
...
def gen_load(config, policy_type, location):
    if policy_type in ADAP_TYPES:
        if "latent_val" not in config:
            raise EnvException(
                "latent_val needs to be specified for \
                                FIXED ADAP policy"
            )
...
def generate_partners(altenv, env, ego, args):
    ...
    for i in range(len(args.alt)):
        args.partner_num = i
        v = gen_partner(args.alt[i], args.alt_config[i], altenv, ego, args)
        print(f"Partner {i}: {v}")
...
def preset(args, preset_id):
    ...
    if preset_id == 1:
        ...
        if args.ego_save is None:
            args.ego_save = "models/%s-%s-ego-%d" % (env_name, args.ego, args.seed)
        if args.alt_save is None:
            args.alt_save = "models/%s-%s-alt-%d" % (env_name, args.alt[0], args.seed)
        # if not args.record:
...
if __name__ == "__main__":
    ...
    parser.add_argument("env", choices=ENV_LIST, help="The environment to train in")

    parser.add_argument("ego", choices=EGO_LIST, help="Algorithm for the ego agent")

    parser.add_argument(
        "alt", choices=PARTNER_LIST, nargs="+", help="Algorithm for the partner agent"
    )
    ...
    parser.add_argument(
        "--device", "-d", default="auto", help="Device to run pytorch on"
    )
    parser.add_argument(
        "--seed", "-s", default=None, type=int, help="Seed for randomness"
    )

    parser.add_argument(
        "--ego-config", type=json.loads, default={}, help="Config for the ego agent"
    )

    parser.add_argument(
        "--alt-config", type=json.loads, nargs="*", help="Config for the ego agent"
    )

    parser.add_argument(
        "--env-config", type=json.loads, default={}, help="Config for the environment"
    )
    ...
    parser.add_argument(
        "--record", "-r", help="Saves joint trajectory into file specified"
    )

    parser.add_argument("--ego-save", help="File to save the ego agent into")
    parser.add_argument("--alt-save", help="File to save the partner agent into")
    ...
    parser.add_argument("--tensorboard-log", help="Log directory for tensorboard")

    parser.add_argument("--tensorboard-name", help="Name for ego in tensorboard")
